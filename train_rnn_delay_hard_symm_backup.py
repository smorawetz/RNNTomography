...
def run_training(
    ...
    training_results_name = (
        "{0}/training_results_rnn_{1}_N{2}_nh{3}_lr{4}_ep{5}_symm_ep{6}_seed{7}.txt"
        .format(
            study_path,
            model_name,
            num_spins,
    ...
    training_model_name = (
        "{0}/rnn_state_{1}_N{2}_nh{3}_lr{4}_ep{5}_symm_ep{6}_seed{7}.pt".format(
            study_path,
            model_name,
            num_spins,
            num_hidden,
    ...
    if track_fid:
        init_nn_probs = rnn_model.probability(model, hilb_space, impose_symm)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if (epoch % period1 == 0 and epoch <= period_crossover) or epoch % period2 == 0:
            if track_fid:
                nn_probs = rnn_model.probability(model, hilb_space, impose_symm)
