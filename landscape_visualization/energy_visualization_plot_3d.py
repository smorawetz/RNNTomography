...
def energy_plot_3d(physics_model, symm_type, N, max_change):
    ...
    landscape_file = (
        "energy_landscape_range{0}_dim{1}_{2}_{3}_seed{4}.txt".format(
    ...
    # ax.plot_surface(grid_alphas, grid_betas, interp_vals)
    ...
    image_name = (
        "plot_energy_landscape_3d_range{0}_dim{1}_{2}_{3}_seed{4}.pdf".format(
            max_change, DIM, model_name, study_name, SEED
        )
    )
    plt.savefig(
