...
def multi_loss_plot_heatmap(physics_models, symm_types, Ns, max_changes):
    ...
    for i in range(len(physics_models)):
        ...
        spacing = 0.5 if max_change >= 1.0 else 0.25
        xticks = np.arange(-10, 10, spacing).tolist()
        yticks = np.arange(-10, 10, spacing).tolist()
    ...
    plt.savefig("compare_results/{0}/compare_loss_heatmap.pdf".format(compare_name))
