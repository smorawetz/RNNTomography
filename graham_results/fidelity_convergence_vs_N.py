...
MARGIN = 0.00  # margin to come within converged error to count as that
...
for model_name in MODEL_NAMES:
    ...
    for N in N_VALS:
        ...
        fidelities = np.loadtxt(vals_file)[:, 1]
        conv_fid = np.mean(fidelities[-TAKE_LAST:])
        if len(np.argwhere(fidelities > (1 - MARGIN) * conv_fid)) == 0:
            first_index = -1
        else:
            first_index = np.argwhere(fidelities > (1 - MARGIN) * conv_fid)[0].item()
...
plt.savefig("compare_results/all_compare/fidelity_convergence_vs_N.pdf".format(LR))
