# ------------------- IN THIS VERSION --------------------
# ---- OPTION TO MANUALLY INITIALIZE WAVEFUNC PARAMS -----
...
def fidelity(target_state, wavefunction, nn_probs):
    """
...
def prob_diff(target_state, wavefunction, nn_probs):
    """
        target_state:   torch.Tensor
                        know state coefficients ordered in computational basis
        wavefunction:   PositiveWaveFunction
...
def KL_div(target_state, wavefunction, nn_probs):
    """
        target_state:   torch.Tensor
    ...
    """
    targ_probs = torch.pow(torch.abs(target_state), 2)  # get probs by squaring coeffs
