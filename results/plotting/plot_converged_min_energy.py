...
def plot_converged_min_energy(model_name, num_spins, num_hidden, lr, num_epochs, N_list):
    """
        model_name: str
    ...
    for N in N_list:
        ...
        avg_fid = np.min(data[-avg_over:, 1])
        final_fid_list.append(avg_fid)
        avg_energy = np.min(data[-avg_over:, 3])
    ...
    plt.savefig("{0}/conv_min_energy_{1}.png".format(study_path, study_name))
